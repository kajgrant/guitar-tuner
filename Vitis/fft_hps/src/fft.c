...
void filter_fft(fft_t *p_fft_inst)
{
	...
	for (ii = 0; ii < p_fft_inst->num_pts; ii++)
	{
		if (ii > CUTOFF_FREQ)
		{
			fft_result[ii].data_re = 0;
			fft_result[ii].data_im = 0;
		}
	}
}
...
void fft_print_normalized(fft_t *p_fft_inst)
{
	...
	tmp = (short *)dma_accel_get_convert_buf(p_fft_inst->periphs.p_dma_accel_inst);

	for (ii = 0; ii < p_fft_inst->num_pts; ii++)
	{
		cplx_data_get_normalized_string(str, tmp[ii]);
		xil_printf("Xk(%d) = %s\n\r", ii, str);
	}
}
